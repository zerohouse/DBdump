...
public class ExportMysql implements ExportQuery {
    ...
    private static final String MAKE_QUERY = "SET @query = CONCAT(\"select * from ((SELECT \",@columnNames,\") UNION (SELECT * FROM `\",@tableName,\"`)) as a\tINTO OUTFILE '\", @outputFile, \"' fields TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\\\"' ESCAPED BY '' LINES TERMINATED BY '" + System.getProperty("line.separator") + "'\");";
